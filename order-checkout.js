...
const products={"pompkin-pack":{name:"Allynieva Pompkin Pack",price:249,weight:100},"acrylic-cd-keychain":{name:"Acrylic CD Album Keychain with NFC",price:170,weight:35},"sanlias-print":{name:"SanLias 4R Mini Art Print",price:70,weight:20},"allynieva-sticker-sheet":{name:"Allynieva Sticker Sheet",price:70,weight:15}};
const money=v=>"₱"+Math.max(0,Number(v)||0).toLocaleString("en-PH");
...
function renderAddresses() {
  ...
  box.innerHTML = `
    <div class="checkout-address-picker">
      ...
      <div class="checkout-address-picker-row">
        ...
        <label class="checkout-address-select-wrap" for="savedAddressSelect">
          <span class="checkout-address-icon">⌂</span>

          <span class="checkout-address-select-text">
            <strong>
              ${selectedAddress.address_label || "Saved address"}
              ${selectedAddress.is_primary ? " — Primary" : ""}
            </strong>
            <small>
              ${selectedAddress.recipient_name || "Recipient"}
              ·
              ${selectedAddress.mobile_number || "Mobile not set"}
            </small>
          </span>

          <span class="checkout-address-chevron">⌄</span>

          <select id="savedAddressSelect" aria-label="Choose saved address">
            ${addresses.map(a => `
              <option
                value="${a.id}"
                ${a.id === selectedAddress.id ? "selected" : ""}
              >
                ${a.address_label || "Saved address"}${a.is_primary ? " — Primary" : ""}
              </option>
            `).join("")}
          </select>
        </label>

        <a class="checkout-manage-addresses" href="profile.html">
          Manage addresses
        </a>
  ...
  document
    ...
    .addEventListener("change", e => {
      selectedAddress =
        addresses.find(a => a.id === e.target.value) || primary;
    ...
    <article class="checkout-selected-address">
      ...
      <div class="checkout-selected-address-header">
        ...
        <div class="checkout-selected-address-name">
          <span class="checkout-address-circle">⌂</span>
        ...
        <a href="profile.html" class="checkout-change-address">
          Change address
        </a>
      ...
      <div class="checkout-address-info">
        ...
        <div>
          ...
          <span class="checkout-info-label">Recipient</span>
          <strong>${a.recipient_name || "Not set"}</strong>
        </div>

        <div>
          <span class="checkout-info-icon">⌕</span>
          <span class="checkout-info-label">Mobile</span>
          <strong>${a.mobile_number || "Not set"}</strong>
        </div>

        <div class="checkout-address-full">
          <span class="checkout-info-icon">⌖</span>
          <span class="checkout-info-label">Delivery address</span>
          <strong>${a.full_address || "No delivery address saved."}</strong>
        </div>
...
async function init(){const {data:{session:s}}=await supabase.auth.getSession();session=s;if(!session){localStorage.setItem("pompkin_auth_return","/Pompkin/order-checkout.html");location.href="login.html";return;}try{await loadData();renderItems();renderAddresses();}catch(e){msg(`We couldn't load your saved checkout information: ${e.message}`,"error");}}
init();
